            ...
            <form @submit.prevent="submit">
                <div class="px-8 py-6">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <!-- Item Selection -->
                        <div class="flex flex-col gap-2">
                            <label class="block text-md font-bold">SELECT ITEM</label>
                            <div class="flex gap-4">
                                <!-- All Item Option -->
                                <label class="inline-flex items-center cursor-pointer group">
                                    <input type="radio" v-model="form.item_type" value="All Items"
                                        class="hidden peer" />
                                    <div class="relative flex items-center justify-center p-2">
                                        <!-- Hover circle -->
                                        <div class="absolute -inset-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-[var(--color-border)]/40"
                                        ...
                                        <span class="text-sm font-medium z-10">All Items</span>
                                    </div>
                                </label>

                                <!-- By Item Option -->
                                <label class="inline-flex items-center cursor-pointer group">
                                    <input type="radio" v-model="form.item_type" value="By Items" class="hidden peer" />
                                    <div class="relative flex items-center justify-center p-2">
                                        <!-- Hover circle -->
                                        <div class="absolute -inset-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-[var(--color-border)]/40"
                                            :class="{
                                        ...
                                        <span class="text-sm font-medium z-10">By Items</span>
                                    </div>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-1">
                            <label class="block text-md font-bold">SELECT ITEMS</label>
                            <div class="relative">
                                <!-- Selected Tags Display -->
                                <div
                                    class="flex flex-wrap gap-2 p-2 min-h-12 border border-[var(--color-border)] rounded-lg">
                                    <span v-if="form.item_type === 'By Items'" v-for="(
item, index
                                        ) in form.selectedItems" :key="item"
                                        class="inline-flex items-center px-3 py-1 rounded-full bg-[var(--color-primary)]/40 font-medium text-sm">
                                        {{ item }}
                                        <button type="button" @click.stop="removeItem(index)" class="ml-2">
                                            <svg-icon type="mdi" :path="mdiClose" class="w-3 h-3" />
                                        </button>
                                    </span>
                                </div>
                                <TextInput type="text" v-model="itemSearch" @input="searchItems" @focus="openDropdown"
                                    @blur="closeDropdown" @keydown.enter.prevent="
                                        addFirstMatchingItem
                                    " @keydown.down="highlightNext" @keydown.up="highlightPrev"
                                    :modifiedPlaceholder="'By Items Only'" :defaultPlaceholder="'Click To Select Items'"
                                    :readonly="form.item_type === 'All Items'" ref="itemInput" validation="no"
                                    selectable="yes" />
                                ...
                                <div v-show="showItemDropdown"
                                    ...
                                    <div
                                        ...
                                        <ul>
                                            <li v-for="(
item, index
                                                ) in filteredItems" :key="item" @click="selectItem(item)" @mouseover="
                                                ...
                                                {{ item.name }}
                                                <span v-if="isSelected(item)" class="ml-auto">
                        ...
                        <!-- Date Type Selection -->
                        <div class="flex flex-col gap-2">
                            <label class="block text-md font-lg font-bold">SELECT DATE</label>
                        </div>
