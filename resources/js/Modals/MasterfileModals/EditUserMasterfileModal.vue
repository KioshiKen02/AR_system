                ...
                <div v-else class="flex flex-col lg:flex-row">
                    ...
                    <div class="w-full lg:w-[60%] p-4 sm:pl-6 sm:pr-6 sm:pb-6 sm:pt-2">
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <TextInput label="Confirm Password" v-model="form.password_confirmation" type="password" readonly />
...
const fetchEmployeeData = async () => {
    try {
        ...
        if (response.data && (response.data.data?.employee?.length > 0 || response.data.employee?.length > 0)) {
            ...
            // Prefill form fields
            form.employee_id = employeeData.value.employee_id;
...
const submit = () => {
    ...
    form.transform(() => payload).put(route("user-masterfile.update", { tenant: page.props.tenant, user: props.user.id }), {
        preserveScroll: true,
        onSuccess: () => {
