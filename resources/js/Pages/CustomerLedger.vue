<template>
    <div>
        ...
        <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
            ...
            <ViewCustomerLedger v-if="showViewModal" :show="showViewModal" :selected="selectedRow"
                :selectedcustname="selectedcusname" @close="closeViewModal" />
        </Transition>
        <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
            enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-200"
            leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
            <PaymenHistory v-if="showViewPaymentHistoryModal" :customer_code="selectedPaymentRowCustomerCode"
                :document_no="selectedPaymentRowDocumentNO" :type="selectedPaymentRowType"
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center pb-3 pt-1 gap-4">
            <div class="w-full lg:w-auto lg:flex-shrink-0">
                <div
                    ...
                    <span class="text-base font-semibold text-left">Amount Forwarded :
                    </span>
            ...
            <div
                ...
                <div class="relative w-full sm:w-64 lg:w-72">
                    <input type="search" id="Search" v-model="search" placeholder=" " class="peer w-full"
                    ...
                    <button :disabled="(!isLoading &&
                        ...
                        <div class="relative flex items-center justify-center gap-2">
                            <span class="transition-transform duration-300 group-hover:rotate-360">
                                <svg-icon type="mdi" :path="mdiMicrosoftExcel" class="w-5 h-5" />
                            </span>
                        ...
                        <div v-if="showFilters"
                            ...
                            <div class="p-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div>
                                        <label class="block text-xs mb-1">Customer Code</label>
                                        <TextInput type="text" v-model="cus_code" @click="onCustomerClick()"
                                            selectable="yes" />
                                    </div>
                                </div>
                            ...
                            <div
                                ...
                                <button @click="applyFilters" :disabled="!hasActiveFilters"
        ...
        <div class="bg-[var(--color-bg-secondary)]/20 p-4 rounded-md shadow-[0_0_20px_var(--color-shadow)]/20 mt-4">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3">
                <label class="inline-flex items-center gap-2 text-sm text-[var(--color-text-primary)]">
                    <input type="checkbox" v-model="hideFloatingDeducted" data-testid="hide-floating"
                ...
                <div v-if="generateTableData && shouldUseInfiniteScroll && filteredRows.length > 0"
            ...
            <div ref="scrollContainer" class="max-h-[65vh] overflow-auto rounded-xl">
                <table class="w-full text-sm text-[var(--color-text-primary)] rounded-xl overflow-hidden mb-2">
                    ...
                    <tbody>
                        <!-- Loading State -->
                        <tr v-if="isLoading && generateTableData">
                        ...
                        <tr v-else-if="
                            ...
                            filteredRows.length !== 0 &&
                            generateTableData
                        " v-for="(customerledger, idx) in renderedRows" :key="customerledger.id ?? idx"
                        ...
                        ">
                            <td colspan="8" class="px-5 py-4 text-center">
                                <div ref="loadMoreSentinel" class="text-xs text-[var(--color-text-secondary)]">
                                    Loading more rows...
                                </div>
                            </td>
                        </tr>
...
<script setup>
    ...
    const ledgerDisplayMap = computed(() => {
        ...
        filteredRows.value.forEach((row, index) => {
            ...
            const isPaymentType = row?.type === 'Payment';
            ...
            if (isPaymentType) {
                ...
                displayCredit = credit - overpay;
                runningAdjustment += overpay;
            }

            const displayRunningBalance = Math.max(
                0,
                toNumber(row?.running_balance) + (isPaymentType ? 0 : runningAdjustment)
            );
