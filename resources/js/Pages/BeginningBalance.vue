    ...
    <div>
        ...
        <div class="flex justify-between pb-3 pt-1">
            <div class="flex gap-2">
                <button v-if="canInsert('0204-BGBLT')" @click="openModal()"
                ...
                <button :disabled="!canView('0204-BGBLT')" @click="openImportExportModal()"
                    class="px-4 py-2 rounded-md font-medium transition-all duration-300 flex items-center justify-center gap-2 relative overflow-hidden bg-[var(--color-primary)] text-white hover:bg-transparent hover:text-[var(--color-primary)] hover:ring-1 hover:ring-[var(--color-primary)] disabled:opacity-70 disabled:cursor-not-allowed group">
                    <div class="relative flex items-center justify-center gap-1">
                        <span class="transition-transform duration-300 group-hover:rotate-360">
                            <svg-icon type="mdi" :path="mdiFileDocumentPlusOutline" class="w-5 h-5" />
                        </span>
        ...
        <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
            ...
            <AddBeginningBalance v-if="showModal" :show="showModal" @close="closeModal"
                @closeSuccess="closeSuccessModal" />
        </Transition>
        <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
            enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-200"
            leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
            <ImportExportBegBal v-if="showImportExportModal" :show="showImportExportModal"
                @close="closeImportExportModal" @closeSuccess="closeSuccessImportExportModal" />
        </Transition>
        <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
            enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-200"
            leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
            <ViewBeginningBalance v-if="showViewModal" :show="showViewModal" :selected="selectedRow"
        ...
        <div class="bg-[var(--color-bg-secondary)]/20 p-4 rounded-md shadow-[0_0_20px_var(--color-shadow)]/20 mt-4">
            <table class="w-full text-sm text-[var(--color-text-primary)] rounded-xl overflow-hidden mb-2">
                <thead class="sticky top-0 z-10">
                    <tr>
                        <th class="px-3 py-3 w-[20%] text-left font-semibold tracking-wider uppercase">
                            Beginning Balance No
                        </th>
                        <th class="px-3 py-3 w-[20%] text-left font-semibold tracking-wider uppercase">
                            Receipt Date
                        </th>
                        <th class="px-3 py-3 w-[35%] text-left font-semibold tracking-wider uppercase">
                            Customer Name
                        </th>
                        <th class="px-3 py-3 w-[15%] text-center font-semibold tracking-wider uppercase">
                            Balance
                        </th>
                        <th class="px-3 py-3 w-[10%] text-center font-semibold tracking-wider uppercase">
                            Action
                        </th>
                    </tr>
                </thead>
                ...
                <!-- Modern Body -->
                <tbody v-else>
                    <tr v-for="beginningbalance in beginningbalances.data" :key="beginningbalance.id"
                        ...
                        <td class="px-3 py-1">
                            <div class="flex justify-center gap-2">
                                <button @click="openViewModal(beginningbalance)"
                    ...
                    <!-- Empty State -->
                    <tr v-if="!isLoading && beginningbalances.data.length === 0">
