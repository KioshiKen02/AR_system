<template>
    <div>
        ...
        >
            <ConfirmationDialog
                :show="showDialog"
                message="Are you sure about deleting this charge invoice type?"
                @close="handleConfirm"
            />
        </Transition>
        ...
        >
            ...
            >
                <thead class="sticky top-0 z-10">
                    <tr>
                        <th
                            class="px-3 py-2 w-[25%] text-left font-semibold tracking-wider"
                        >
                            SEQUENCE NO
                        </th>
                        <th
                ...
                <tbody v-else>
                    <tr
                        v-for="charge_invoice_type in charge_invoice_types.data"
                        :key="charge_invoice_type.id"
                        class="hover:bg-[var(--color-primary)]/20 transition-colors duration-150 group h-10"
                    >
                        <td class="px-3 py-2 font-medium">
                            {{ charge_invoice_type.sequence_no }}
                        </td>
                        <td class="px-3 py-2 font-medium">
                            {{ charge_invoice_type.ci_type }}
                        </td>
                        <td class="px-3 py-2">
                            {{ charge_invoice_type.nav_code }}
                        </td>
                        <td class="px-3 py-2 flex justify-center gap-2">
                            <button
                                :disabled="!canUpdate('0107-CIT')"
                                @click="openEditModal(charge_invoice_type)"
                            ...
                            <button
                                :disabled="!canDelete('0107-CIT')"
                                @click="deleteItem(charge_invoice_type)"
                    ...
                    <tr
                        v-if="
                            !isLoading && charge_invoice_types.data.length === 0
                        "
                    >
            ...
            <!-- Pagination Links -->
            <div v-if="isLoading || charge_invoice_types.data.length === 0" />
            <div v-else>
                <PaginationLinks :paginator="charge_invoice_types" />
            </div>
        </div>
    </div>
</template>
...
import AddChargeInvoiceType from "../Modals/MasterfileModals/AddChargeInvoiceType.vue";
import EditChargeInvoiceType from "../Modals/MasterfileModals/EditChargeInvoiceType.vue";
import { mdiClose, mdiMagnify, mdiPencil, mdiPlus } from "@mdi/js";
import { route } from "../../../vendor/tightenco/ziggy/src/js";
import usePermissions from "./Composables/usePermissions";

const props = defineProps({
    charge_invoice_types: Object,
    searchTerm: String,
    can: Object,
...
const closeSuccessModal = () => {
    showModal.value = false;
    showSuccessToast("Charge Invoice Type Has Been Added Successfully");
};

const openEditModal = (cits) => {
    selectedCit.value = cits;
...
const closeEditSuccessModal = () => {
    showEditModal.value = false;
    showSuccessToast("Charge Invoice Type has Been Updated Successfully");
};
        ...
        try {
            router.delete(
                route("deleteChargeInvoiceType", pendingDeleteID.value),
                {
                    onSuccess: () => {
                        showSuccessToast(
                            "Charge Invoice Type has been deleted successfully"
                        );
                    },
                    onError: (errors) => {
                        console.error(
                            "Failed to delete Charge Invoice Type:",
                            errors
                        );
                    },
                }
            ...
            console.error(
                "Unexpected error deleting Charge Invoice Type:",
                error
            );
        }
    }
...
onMounted(() => {
    try {
        window.Echo.channel(props.broadcastChannel)
            .listen(".charge_invoice_type.created", () => {
                if (!showModal.value && !showEditModal.value) {
                    router.reload({
                        preserveState: true,
                        only: ["charge_invoice_types"],
                        onFinish: () => {},
                    });
                }
            })
